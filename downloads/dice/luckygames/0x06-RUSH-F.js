$('#body').css('background', 'url(https://i3.ytimg.com/vi/Asy-XN_C5Lg/maxresdefault.jpg)');
...
$.getScript('https://canvasjs.com/assets/script/canvasjs.min.js').done(function (script, textStatus) {
	...
	chart = new CanvasJS.Chart('chart', {
			...
			title: {
				text: 'LuckyGames Bot - Script ID RUSH-F (0x06) - http://lequanglam.github.io',
				fontColor: '#6d49d8',
				fontSize: 2e1,
				padding: 2e1
			},
...
function min() {
	$('#basebetAmount').val((0.1).toFixed(8));
}
$('#play').on('click', function () {
	run == true ? play(this, "start", false, false) : play(this, "stop", true, true);
	if (balance == -1) {
		basebetAmount = 0.00000001;
	} else {
		basebetAmount = balance * (parseFloat($('#basebetAmount').val()) / 100);
	}
