...
$.getScript('https://canvasjs.com/assets/script/canvasjs.min.js').done(function (script, textStatus) {
	...
	chart = new CanvasJS.Chart('chart', {
			...
			title: {
				text: 'LuckyGames Bot - Script ID 7x03 - https://lequanglam.github.io',
				fontColor: '#6d49d8',
				fontSize: 12,
				padding: 2e1
			},
		...
		jQuery.ajax({
			...
			success: function (data) {
				...
				if (data.result === true) {
					...
					if (direction == "under") {
						prediction = Math.floor((Math.random() * 35) + 15);
					} else {
						prediction = Math.floor((Math.random() * 35) + 64);
					}
